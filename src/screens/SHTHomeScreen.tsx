...
const TotalStaked = ({ state }: { state: HomeState }) => {
    ...
    const staked = state.totalStakedBSCH;
    const loading = state.loadingTotalStaked;
    return (
        <View>
            <Heading text={t("total-staked-bsch")} buttonText={t("stake")} onPressButton={goToSwap} />
            <Title
...
const TotalMiningPower = ({ state }: { state: HomeState }) => {
    const t = useTranslation();
    const loading = state.loadingTotalStaked;
    const power = state.totalStakedBSCH!=undefined?
        state.totalStakedBSCH.div(ethers.BigNumber.from(10)):undefined;
    return (
        <View>
