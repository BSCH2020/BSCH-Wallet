...
import useBSCHRoll from "./useBSCHRoll";
...
const useMigrateState: () => MigrateState = () => {
    ...
    const { migrate, migrateWithPermit } = useBSCHRoll();
        ...
        if (provider && signer && state.selectedLPToken) {
            ...
            try {
                const minAllowance = ethers.BigNumber.from(2)
                    .pow(96)
                    .sub(1);
                const allowance = await getTokenAllowance(state.selectedLPToken.address, BSCH_ROLL);
