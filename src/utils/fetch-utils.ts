import { FACTORY_ADDRESS as BSCHSWAP_FACTORY, Pair } from "@bschswap/sdk";
...
export const fetchPools = async (account: string, tokens: Token[], provider: ethers.providers.JsonRpcProvider) => {
    const info = await bschData.bsch.info();
    const masterchefInfo = await bschData.masterchef.info();
    const pools = await bschData.masterchef.pools();
    const reduce = await bschData.masterchef.pool({ poolId: "45" });
    if (!reduce) return undefined;
    const bschPerBlock = Math.floor(100 - 100 * (reduce.allocPoint / masterchefInfo.totalAllocPoint));
    const balances = await fetchTokenBalances(
        account,
    ...
    const fetchPool = async (pool, i): Promise<LPToken | null> => {
        ...
        try {
            ...
            return {
                ...
                balance: ethers.BigNumber.from(balances[i] || 0),
                bschRewardedPerYear: calcBSCHRewardedPerYear(
                    bschPerBlock,
                    pool.allocPoint,
                    masterchefInfo.totalAllocPoint,
                    result[0].totalSupply
    ...
    const fetchMyPool = async (pool): Promise<LPToken | null> => {
        try {
            ...
            return {
                ...
                pendingBSCH: myStake.pendingBSCH,
                totalSupply: parseBalance(String(result[0].totalSupply), 18)
            };
        } catch (e) {
            return null;
...
const calcAPY = (derivedETH, bschPerBlock, allocPoint, totalAllocPoint, totalValueETH, slpBalance, totalSupply) => {
    return (
        (derivedETH * blocksPerDay * bschPerBlock * 3 * 365 * (allocPoint / totalAllocPoint)) /
        (totalValueETH * (slpBalance / totalSupply))
    );
};

const calcBSCHRewardedPerYear = (bschPerBlock, allocPoint, totalAllocPoint, totalSupply) => {
    return ethers.BigNumber.from(blocksPerDay * bschPerBlock * 3 * 365 * allocPoint)
        .mul(pow10(36))
        .div(totalAllocPoint)
...
const fetchStakedValue = async (lpToken: string) => {
    return await bschData.masterchef.stakedValue({ lpToken });
};
...
const fetchMyStake = async (poolId: number, account: string, provider: ethers.providers.JsonRpcProvider) => {
    ...
    const pendingBSCH = await masterChef.pendingBSCH(poolId, account);
    return { amountDeposited, pendingBSCH };
};
...
export const fetchMyLPTokens = async (account: string, tokens: Token[], provider: ethers.providers.JsonRpcProvider) => {
    return await fetchLPTokens(BSCHSWAP_FACTORY, account, tokens, provider);
};
